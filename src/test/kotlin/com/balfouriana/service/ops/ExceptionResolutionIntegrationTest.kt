...
class ExceptionResolutionIntegrationTest {
    @Autowired
    ...
    @Test
    fun `resolve removes item from default open queue`() {
        ...
        mockMvc.perform(
            post("/demo/exceptions/$queueItemId/resolve")
                ...
                .content("""{"resolutionType":"DISMISSED","resolvedBy":"operator"}""")
        )
            .andExpect(status().isOk)
            .andExpect(jsonPath("$.queueItemId").value(queueItemId.toString()))
            .andExpect(jsonPath("$.resolutionType").value("DISMISSED"))

        assertTrue(exceptionQueueQueryService.openExceptions(correlationId = ruleEvent.metadata.correlationId).isEmpty())
        assertEquals(
            1,
            exceptionQueueQueryService.openExceptions(
                correlationId = ruleEvent.metadata.correlationId,
    ...
    @Test
    fun `resubmit after linked nack emits new submission and resolves queue item`() {
        ...
        assertTrue(exceptionQueueQueryService.openExceptions(correlationId = correlationId).isEmpty())
        assertEquals(
            2,
            eventStoreRepository.findByCorrelationId(correlationId).count { it.eventType == "FilingSubmittedEvent" }
        )
    }

    @Test
    fun `resubmit without filing ready returns bad request`() {
        ...
        mockMvc.perform(
            post("/demo/exceptions/$submissionFailedEventId/resubmit")
                .contentType(MediaType.APPLICATION_JSON)
                .content("""{"resolvedBy":"operator"}""")
        )
            .andExpect(status().isBadRequest)

        assertFalse(eventStoreRepository.hasResolutionForQueueItem(submissionFailedEventId))
    }

    @Test
    fun `demo exceptions endpoint supports include resolved`() {
        ...
        mockMvc.perform(get("/demo/exceptions").param("correlationId", ruleEvent.metadata.correlationId.toString()))
            .andExpect(status().isOk)
            .andExpect(jsonPath("$").isEmpty)

        mockMvc.perform(
            get("/demo/exceptions")
                .param("correlationId", ruleEvent.metadata.correlationId.toString())
                .param("includeResolved", "true")
        )
            .andExpect(status().isOk)
            .andExpect(jsonPath("$[0].status").value("RESOLVED"))
            .andExpect(jsonPath("$[0].resolutionType").value("ACKNOWLEDGED"))
    }
